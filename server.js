...
// Define the updatePoolInfo function
async function updatePoolInfo(direction, amount, estimatedAmount, transactionFee) {
    try {
...
app.post("/api/updatePoolInfo", async (req, res) => {
    try {
        ...
        // Find the latest pool info
        const latestPoolInfo = await PoolInfo.findOne().sort({ timestamp: -1 });

        // Update the pool info with the new amounts
